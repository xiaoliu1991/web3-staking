...
export const WalletHandlerProvider = ({children})=>{
    ...
    const claimAirDrop = async()=>{
        try{
            ...
            //调用合约
            const airdropContract = getEthereumContract(airContractAddress,airContractABI);
            const isClaimed = await airdropContract.isClaimed(connectedAccount);
            if(isClaimed){
                alert("已经领取了");
                return;
            }
            const tx = await airdropContract.claimToken();
            await tx.wait();
            console.log(tx);
        }catch(error){
            console.log(error);
