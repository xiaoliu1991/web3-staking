...
contract Staking {
    ...
    mapping(address => uint256) public userRewards; //玩家每次请求计算奖励后的存的数据,用户显示

    event Staked(address indexed user, uint256 amount);//质押事件
    event RewardUpdate(address indexed user, uint256 amount);//奖励更新事件
